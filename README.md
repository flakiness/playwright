...
For stable CI sharding, fetch a timings snapshot once and make every shard use
that same file:

```bash
npx flakiness-playwright-timings fetch --output=.flakiness/timings.json --project=chromium tests/e2e
npx flakiness-playwright-shard --shard=1/2 --timings=.flakiness/timings.json --project=chromium tests/e2e
npx flakiness-playwright-shard --shard=2/2 --timings=.flakiness/timings.json --project=chromium tests/e2e
```

### Sharding from a local timings file

By default durations are fetched from the Flakiness.io Durations API. Pass `--timings=<file>` to balance shards from a local Flakiness report instead — for example one generated by `flakiness-playwright-timings fetch`, or a previous run's `flakiness-report/report.json`:

```bash
npx flakiness-playwright-shard --shard=1/2 --timings=./flakiness-report/report.json
```

This avoids the network round-trip and works without a Flakiness.io token — handy in CI when you carry the last run's report forward as an artifact, or when iterating offline. Tests missing from the file fall back to a default weight, so a stale or partial timings file still produces a valid (if less balanced) split.
